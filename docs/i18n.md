...
### Using ThemedText with Translations

For text that needs proper theming and translations:
...
function MyComponent() {
  ...
  return <ThemedText>{t("common.loading")}</ThemedText>;
}
```
...
On web platforms, the system automatically handles:
...
3. **Use the ThemedText component for user-facing text**: This ensures proper theming and accessibility
...
- Use the `ThemedText` component for text that needs proper theming
